  ...
  const handlePointerMove = (event: ReactPointerEvent<SVGRectElement>) => {
    ...
    for (const [index, station] of stations.entries()) {
      ...
      readings.push({
        ...
        name: station.name,
        color: STATION_CHART_COLORS[index % STATION_CHART_COLORS.length]!,
        eColi: point.eColi,
    ...
    if (readings.length === 0 || !nearestDateTime) {
      setHover(null);
      return;
    }
  ...
  return (
    <div className={`timeline-chart-wrap${narrow ? " is-narrow" : ""}`}>
          ...
          <ul className="timeline-tooltip-list">
            {hover.readings.map((reading) => (
              <li key={reading.stationId}>
                <span className="timeline-tooltip-swatch" style={{ background: reading.color }} />
      ...
      <svg
        ...
        role="img"
        aria-label={`Estimated E. coli levels over the past ${HISTORY_PERIODS.find((item) => item.value === period)?.label ?? period}`}
      >
...
export function EcoliTimeline({
  ...
  return (
    <section className="timeline-section" aria-labelledby="timeline-heading">
      <p id="timeline-heading" className="visually-hidden">
        E. coli timeline
      </p>
      ...
      <div className="timeline-controls" role="tablist" aria-label="History range">
      ...
      <div className="timeline-panel">
        {loading && <div className="timeline-skeleton" aria-hidden="true" />}

        {!loading && error && <p className="timeline-error">{error}</p>}
              ...
              {history.stations.map((station, index) => (
                <li key={station.id}>
                  <span
                    ...
                    style={{ background: STATION_CHART_COLORS[index % STATION_CHART_COLORS.length] }}
                  />
                  {station.name}
                </li>
              ))}
        ...
        {!loading && !error && history && !hasData && (
          <p className="timeline-error">No historical readings for this range.</p>
        )}
      </div>
    </section>
  );
